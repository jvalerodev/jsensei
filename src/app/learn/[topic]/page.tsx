...
import { Loader2, BookOpen, Code, CheckCircle, XCircle, Lightbulb, Play } from "lucide-react"
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-slate-900">
                {topicTitles[topic] || topic.charAt(0).toUpperCase() + topic.slice(1)}
              </h1>
              <p className="text-slate-600">Contenido personalizado para tu nivel</p>
            </div>
          </div>
          ...
          <div className="space-y-8">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Explicación detallada del concepto</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="prose prose-slate max-w-none">
                  <div className="whitespace-pre-wrap text-slate-700 leading-relaxed">{content.content}</div>
                </div>
              </CardContent>
            </Card>
                ...
                <CardContent>
                  <div className="space-y-8">
                    {content.exercises.map((exercise, index) => (
                      <div key={exercise.id} className="border rounded-lg p-6 bg-white">
                        <div className="flex items-center justify-between mb-4">
                          <h4 className="font-medium text-slate-900">Ejercicio {index + 1}</h4>
                        ...
                        {exercise.type === "multiple-choice" && exercise.options && (
                          <RadioGroup
                            ...
                            onValueChange={(value) => setUserAnswers((prev) => ({ ...prev, [exercise.id]: value }))}
                            className="mb-4"
                          >
                            {exercise.options.map((option, optionIndex) => (
                              <div key={optionIndex} className="flex items-center space-x-2">
                                <RadioGroupItem value={option} id={`${exercise.id}-${optionIndex}`} />
                                <Label htmlFor={`${exercise.id}-${optionIndex}`} className="text-slate-700">
                                  {option}
                                </Label>
                              </div>
                            ))}
                        ...
                        <div className="flex items-center gap-3">
                          <Button
                            ...
                            disabled={!userAnswers[exercise.id] || showResults[exercise.id]}
                            className="bg-blue-600 hover:bg-blue-700"
                          >
                            {showResults[exercise.id] ? "Respondido" : "Enviar Respuesta"}
                          </Button>

                          {showResults[exercise.id] && exerciseResults[exercise.id] && (
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className={`text-sm font-medium ${
                                  exerciseResults[exercise.id].isCorrect ? "text-green-600" : "text-red-600"
                                }`}
                              >
                                {exerciseResults[exercise.id].isCorrect ? "¡Correcto!" : "Incorrecto"}
                              </span>
                            </div>
                          )}
                        </div>
                        ...
                        {showResults[exercise.id] && exerciseResults[exercise.id] && (
                          <div className="mt-4 p-4 rounded-lg bg-slate-50 border-l-4 border-blue-500">
                            <p className="text-slate-700 mb-2">{exerciseResults[exercise.id].feedback}</p>
                            <p className="text-sm text-slate-600 font-medium">Explicación:</p>
                            <p className="text-sm text-slate-600">{exercise.explanation}</p>
                            {exerciseResults[exercise.id].suggestions && (
                              <div className="mt-2">
                                <p className="text-sm text-slate-600 font-medium">Sugerencias:</p>
                                <ul className="text-sm text-slate-600 list-disc list-inside">
                                  {exerciseResults[exercise.id].suggestions.map((suggestion, idx) => (
            ...
            <div className="flex gap-4 justify-center">
              <Button onClick={generateContent} variant="outline" className="bg-white">
                Generar Nuevo Contenido
              </Button>
              <Button className="bg-blue-600 hover:bg-blue-700">Continuar al Siguiente Tema</Button>
            </div>
          </div>
        )}
      </div>
